/**
 * @file    mycontrol.c
 * @brief   避障、循迹任务，以及手势遥控模式与 500ms 心跳超时保护
 *
...
/* ================== 手势遥控模式（电脑摄像头 → 蓝牙串口） ==================
 * 时间基准用 TIM1 的 1ms 更新中断：TIM2/TIM3/TIM4 已分别被舵机 PWM、
 * 电机 PWM、超声波计时占用，SysTick 被 Delay_us 直接改写寄存器做忙等，
 * 都不能再用；TIM1 原本空闲，且只用更新中断、不占引脚。
 * 计时由硬件中断驱动，不依赖主循环执行速度。
 * 本模式未激活时以下函数不参与电机控制，原有手机遥控 / 循迹 / 避障不受影响。
 * ========================================================================= */
